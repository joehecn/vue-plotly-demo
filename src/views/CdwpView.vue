...
const layout1 = {
  title: 'CDWP Running Status (Carnot)',
  barmode: 'stack',
  yaxis: {
...
const layout2 = {
  title: 'CDWP Running Status (Mega)',
  barmode: 'stack',
  yaxis: {
    tickformat: 'd', // 设置刻度格式为整数
    dtick: 1,        // 设置刻度间隔为 1
...
const refreshData = async () => {
  ...
  try {
    ...
    const dataCarnot = await fetchCarnotData('cdwpRunningStatus', [startTime, _endTime])
    chartData1.value = dataCarnot as ChartDataRow[]

    const dataMega = settingsStore.getMegaData('cdwpRunningStatus', [startTime, _endTime])
