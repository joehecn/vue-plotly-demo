...
const isEqualHeads = (heads: string[]) => {
  const headTemplate = ['时间戳', '冷量需求(kW)', '运行机组', '负载率(%)', '供水温度(°C)', '机组能耗(kW)', 'COP', '水泵_A', '冷却塔_A', '水泵能耗_A(kW)', '冷却塔能耗_A(kW)', '水泵_B', '冷却塔_B', '水泵频频率', '冷却塔转速', '水泵能耗_B(kW)', '冷却塔能耗_B(kW)']
...
const generateColumns = (heads: string[]) => heads.map(head => ({
  key: head,
  dataKey: head,
  title: head,
  width: 150
