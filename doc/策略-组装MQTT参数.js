...
function main() {
  ...
  const supplyTemps = item.supply_temps.map(temp => temp > MAX_SUPPLY_TEMP ? MAX_SUPPLY_TEMP : temp < MIN_SUPPLY_TEMP ? MIN_SUPPLY_TEMP : temp)
  const chs = chillers(item.active_chillers, supplyTemps)
