...
function run(lastStatus, optimize) {
  ...
  const caPlus = chillerA(deviceGroups.ch_a[0], deviceGroups.ch_a[1], optimize.active_chillers, supplyTemps, lastStatus)
  const cbPlus = chillerB(deviceGroups.ch_b[0], deviceGroups.ch_b[1], optimize.active_chillers, supplyTemps)
  const paPlus = pumpA(deviceGroups.cdwp_a[0], deviceGroups.cdwp_a[1])
  const pbPlus = pumpB(deviceGroups.cdwp_b[0], deviceGroups.cdwp_b[1], optimize.cooling_system.group_b.pumps.active, optimize.cooling_system.group_b.pumps.frequency, lastStatus)
  const taPlus = towerA(deviceGroups.ct_a[0], deviceGroups.ct_a[1])
  const tbPlus = towerB(deviceGroups.ct_b[0], deviceGroups.ct_b[1], optimize.cooling_system.group_b.towers.speed)

  // 再减机
  // ch_a, ch_b, cdwp_a, cdwp_b, ct_a, ct_b
  const caSub = chillerA(deviceGroups.ch_a[1], deviceGroups.ch_a[2], optimize.active_chillers, supplyTemps, lastStatus)
  const cbSub = chillerB(deviceGroups.ch_b[1], deviceGroups.ch_b[2], optimize.active_chillers, supplyTemps)
  const paSub = pumpA(deviceGroups.cdwp_a[1], deviceGroups.cdwp_a[2])
  const pbSub = pumpB(deviceGroups.cdwp_b[1], deviceGroups.cdwp_b[2], optimize.cooling_system.group_b.pumps.active, optimize.cooling_system.group_b.pumps.frequency, lastStatus)
  const taSub = towerA(deviceGroups.ct_a[1], deviceGroups.ct_a[2])
  const tbSub = towerB(deviceGroups.ct_b[1], deviceGroups.ct_b[2], optimize.cooling_system.group_b.towers.speed)
